...
def macro(documentevent=None):  # 引数は文書のイベント駆動用。import pydevd; pydevd.settrace(stdoutToServer=True, stderrToServer=True)
	...
	componentwindow.addMouseListener(mouselistener)
# 	doc.addDocumentEventListener(DocumentEventListener(mouselistener))  # DocumentEventListener	
...
# 					numX["Text"] = str(enhancedmouseevent.X)  # プロパティに代入するときは
