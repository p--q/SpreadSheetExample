...
def macro(documentevent=None):  # 引数は文書のイベント駆動用。
	...
	outputs.append(("",))
	outputs.append(("ContainerWindowChild",))
	frame = controller.getFrame()  # フレームを取得。
	containerwindow = frame.getContainerWindow()
	getAccessibleChildren((), containerwindow, outputs)
	outputs.append(("",))
	outputs.append(("FrameChild",))	
	accessiblecontextparent = containerwindow.getAccessibleContext().getAccessibleParent()  # コンテナウィンドウの親AccessibleContextを取得する。
	getAccessibleChildren((), accessiblecontextparent, outputs)
	outputs.append(("",))
	outputs.append(("Frame",))		
	createOutput((), accessiblecontextparent, outputs)
	sheet = getNewSheet(doc, "ChildWindows")  # 連番名の新規シートの取得。OnTitleChanged→OnModifyChangedが呼ばれてしまう。
...
def getAccessibleChildren(head, accessiblecontext, outputs):  # AccessibleContextからAccessibleContextChildを取得。
	...
	for i in range(accessiblecontext.getAccessibleChildCount()):	
		...
		createOutput(head, accessiblechild, outputs)
def createOutput(head, xaccessible, outputs):  # XAccessibleからAccessibleRole、位置、大きさなど取得する。			
	childaccessiblecontext = xaccessible.getAccessibleContext()
