				...
				if target.supportsService("com.sun.star.sheet.SheetCell"):  # ターゲットがセルの時。
					...
					if 0<celladdress.Row<r:  # 行を限定。
						...
						if header=="FilterName":  # 列ヘッダーがFilterNameの列のとき。
							...
							datarow = "Before the dialog.",
							outputs = [datarow]  # 出力行のリスト。
							expandPropertyValueStructs(outputs, filteroptiondialog.getPropertyValues(), 0)  # PropertyValue Structを展開する。							
							if filteroptiondialog.execute()==ExecutableDialogResults.OK:  # フィルターのオプションダイアログを表示。
								filterpropertyvalues = filteroptiondialog.getPropertyValues()  # 戻り値はPropertyValue Structのタプル。XPropertyAccessインターフェイスのメソッド。	
								datarow = "After the dialog.",								
								outputs.append(datarow)
