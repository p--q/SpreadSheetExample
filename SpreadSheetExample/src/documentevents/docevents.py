...
def macro(arg=None):  # 引数は文書のイベント駆動用。  
	...
	if arg.typeName=="com.sun.star.document.DocumentEvent":  # 引数がDocumentEventのとき。
		...
		RESULTS.append((ORDER, eventname, *DIC[eventname]))
	else:  # 引数がDocumentEvent以外のときはない?
		RESULTS.append((ORDER, str(arg)))
	ORDER += 1
def output(doccmentevent=None):  # Calcのシートにイベントの呼び出し順を書き出す。
	macro(doccmentevent)  # このイベント自身の呼び出し順を取得。
	results = RESULTS.copy()  # この時点の結果を出力する。
